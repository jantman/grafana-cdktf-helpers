...
- **`dashboard_builder`** — Pythonic dashboard builder with type-safe dataclasses (`Dashboard`, `Row`, `Target`, `FieldConfig`, etc.) and factory functions for common Home Assistant sensor panels (temperature, humidity, radon). Panel types: `TimeseriesPanel`, `HeatmapPanel` (for data that is already bucketed — pair it with `Target(format="heatmap")`), `XYChartPanel` (scatter, with explicit x/y fields, an optional `color_field` that defaults to a continuous palette, and a `frame_index`; its series config is written in the schemaVersion 38 shape that `Dashboard` declares, because Grafana migrates the dashboard forward on load and double-wraps an already-modern matcher into one that resolves to nothing) and `LogsPanel`. Panels and targets are Prometheus-backed by default but can name any `datasource_type`; `LokiTarget` is a LogQL target with log-appropriate defaults, and `Dashboard` propagates its `datasource_type` to panels that do not set one. Panels also accept `transformations`, which are emitted only when non-empty.
